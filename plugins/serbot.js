...
let handler = async (m, { conn: _conn, args, usedPrefix, command, isOwner }) => {
    ...
    const logout = async () => {
        await parent.sendMessage(conn.user?.jid || m.chat, { text: 'Conexion perdida...' })
    ...
    conn.ev.on('connection.update', async ({ qr, isNewLogin, lastDisconnect }) => {
        if (shouldSendLogin && conn.user) {
            await parent.sendMessage(conn.user.jid, { text: 'Conectado correctamente a su WhatsApp.\n*INFO: esto solo es temporal*\n' + JSON.stringify(conn.user, null, 2) }, { quoted: m })
        }
        if (qr) {
...
handler.help = ['serbottest']
handler.tags = ['serbottest']

handler.command = /^serbottest$/i
